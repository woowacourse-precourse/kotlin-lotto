...
class Lotto(private val numbers: List<Int>) {
    ...
    private fun checkLottoNumberCountException() {
        if (this.numbers.size != LOTTO_NUMBER_COUNT) throw IllegalArgumentException(LOTTO_NUMBER_COUNT_ERROR_MESSAGE)
    }

    private fun checkNumberRangeException() {
        this.numbers.forEach { number ->
            if (number < MIN_NUMBER || number > MAX_NUMBER) throw IllegalArgumentException(
                LOTTO_NUMBER_RANGE_ERROR_MESSAGE
            )
        }
    }

    private fun checkDuplicateNumberException() {
