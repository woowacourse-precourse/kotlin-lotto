...
class InputValidation {
    ...
    fun checkInputSplitWithComma(input: String): List<String> {
        ...
        if (winningNumbers.size != WINNING_NUMBER_COUNT) throw java.lang.IllegalArgumentException(
            SPLIT_WITH_COMMA_ERROR_MESSAGE
        )
        return winningNumbers
    }

    fun checkBonusNumberRange(bonusNumber: Int) {
        if (bonusNumber < Lotto.MIN_NUMBER || bonusNumber > Lotto.MAX_NUMBER) throw IllegalArgumentException(
