...
class LottoHost {
    ...
    fun throwWinningNumber(inputWinningNumber: String) {
        if (!inputWinningNumber.contains(",")) throw IllegalArgumentException("[ERROR] 양식을 지켜주세요. ex)1,2,3,4,5,6")
    }

    fun throwBonusNumber(inputBonusNumber: Int) {
        if (inputBonusNumber !in 1..45) throw IllegalArgumentException("[ERROR] 보너스 번호는 1개의 숫자만 입력 가능합니다")
    }
    ...
    fun throwLottoException(
        ...
        if (winningNumber.size != 6) throw IllegalArgumentException("[ERROR] 당첨 번호는 6개의 숫자만 입력 가능합니다")
        if (winningNumber.distinct().size != 6) throw IllegalArgumentException("[ERROR] 중복된 숫자가 존재합니다")
        ...
        if (winningNumber.contains(bonusNumber)) throw IllegalArgumentException("[ERROR] 당첨번호와 보너스번호는 중복될 수 없습니다.")
    }
}