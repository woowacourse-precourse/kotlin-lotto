...
class Calculator {
    fun compareLottoNum(lottoNum:List<List<Int>>, lotto:Lotto, bonus: Int): List<Int>{
        var score = mutableListOf<Int>(0,0,0,0,0)
        for(i in 0 until lottoNum.size){
            val result = checLotto(lottoNum[i], lotto, bonus)
            when(result){
                Answer.THREE -> score[0]= score[0].plus(1)
                Answer.FOUR -> score[1]= score[1].plus(1)
                Answer.FIVE -> score[2]= score[2].plus(1)
                Answer.BONUS -> score[3]= score[3].plus(1)
                Answer.SIX -> score[4] = score[4].plus(1)
                else->continue
            }
        }
        return score.toList()
    ...
    fun checLotto(lottoNum:List<Int>, lotto:Lotto, bonus: Int) : Answer{
        ...
        when(score){
            ...
            5->{ if(lottoNum.contains(bonus)) return Answer.BONUS
                return Answer.FIVE }
            6->return Answer.SIX
        }
        return Answer.NONE
    ...
    fun calculateYield(amount: Int, score:List<Int>): Double{
        ...
        return round(price / amount / 10.0 *10)/10.0
    }
}