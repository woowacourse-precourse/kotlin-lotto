...
class Lotto(_numbers: List<Int>, _bonus: Int) {
    ...
    fun calcuateRanking(userLottoList:List<List<Int>>) {
        for(element in userLottoList){
            LottoStats.lottoMatchLists(element, numbers)
        }
    }
    fun getSumPrice(userLottoList:List<List<Int>>): Int {
        calcuateRanking(userLottoList)
        var sumPrice = 0
