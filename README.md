...
## 기능 목록
 - 1~45 사이의 숫자 중 중복되지 않는 무작위 6개의 숫자를 뽑아 로또를 발행한다.
 - 구입 금액을 입력받는다.
 - 1,000원 당 로또 1장을 발행한다.
 - 1,000원으로 나누어 떨어지지 않는 금액이라면 예외로 처리한다.
 - 발행한 로또 수량 및 번호를 출력한다. 이때 로또 번호는 오름차순 정렬한다.
 - 당첨 번호와 보너스 번호를 입력받는다.
 - 잘못된 값을 입력(중복되는 수, 1~45 사이가 아닌 수)할 경우 예외로 처리한다.
 - 구매한 로또의 번호와 당첨 번호 + 보너스 번호를 대조하여 일치하는 숫자의 수를 구한다.
 - 일치하는 수의 갯수에 따라 해당 로또를 1~5등 당첨 또는 낙첨으로 분류한다.
 - 당첨 금액과 당첨 수를 출력한다.
 - 수득 금액 / 구입 금액 * 100 = 수익률 
